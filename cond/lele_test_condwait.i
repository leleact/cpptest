...
# 5 "lele_test_condwait.cpp" 2


int nBerun = 1;
pthread_mutex_t mutex = { { 0, 0, 0, 0, 0, 0, 0, { 0, 0 } } };
...
int main() {
  ...
  pthread_mutex_init(&mutex_show, __null);


  for (int i = 0; i < nThreadNum; i++) {
    nRet = pthread_create(&pidThreadDealMess[i], __null, ThreadDealMessage, __null);
    if (nRet) {
  ...
  nRet = pthread_create(&pidGetMess, __null, ThreadDealMessage, __null);
  if (nRet) {
    std::cout << "pthread_create error" << std::endl;
    exit(1);
  }
  ...
  for (int i = 0; i < nThreadNum; i++) {
    pthread_join(pidThreadDealMess[i], __null);
  }
  pthread_join(pidGetMess, __null);
